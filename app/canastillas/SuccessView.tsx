  ...
  return (
    <div className="space-y-8 text-center animate-in zoom-in-95 duration-500">
      <div className="flex flex-col items-center">
        ...
        <h2 className="text-2xl font-black text-slate-900">¡Inventario Guardado!</h2>
        <p className="text-slate-500 mt-1">El registro ha sido procesado exitosamente.</p>
      </div>
      ...
      <div className="bg-white p-8 rounded-3xl shadow-xl shadow-slate-200 border border-slate-100 text-left relative overflow-hidden">
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Reporte #INV-{Math.floor(Math.random() * 90000 + 10000)}</h3>
            <p className="text-xs font-medium text-slate-400">{now}</p>
          </div>
          <div className="bg-blue-600 px-3 py-1 rounded-full text-[10px] font-bold text-white uppercase">Completado</div>
        </div>

        <div className="space-y-4 mb-8">
          <div className="flex justify-between items-center py-2 border-b border-slate-50">
            <span className="text-slate-600">Total Unidades</span>
            <span className="text-xl font-black text-slate-900">{totalCrates}</span>
          </div>
        ...
        <div>
          <h4 className="text-xs font-bold text-slate-400 uppercase mb-3">Firma Digital</h4>
          <div className="bg-slate-50 rounded-2xl border border-slate-200 p-4 h-32 flex items-center justify-center">
            <img src={signature} alt="Firma" className="max-h-full max-w-full grayscale contrast-125" />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <button 
          className="py-4 bg-white border border-slate-200 text-slate-900 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-slate-50 transition-all active:scale-95"
        >
