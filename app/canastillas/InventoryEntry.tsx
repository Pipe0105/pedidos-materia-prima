...
import { CrateType, CrateStatus, InventoryItem } from '../types';
...
export const InventoryEntry: React.FC<Props> = ({ items, onAddItem, onRemoveItem, onNext }) => {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        ...
        <div className="grid grid-cols-1 gap-4">
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">Tipo de Canastilla</label>
            <select 
              value={type}
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider px-2">Lista Actual ({items.length})</h3>
        {items.length === 0 ? (
          ...
          <div className="grid gap-3">
            {items.map(item => (
              <div key={item.id} className="bg-white p-4 rounded-xl border border-slate-200 flex items-center justify-between shadow-sm animate-in zoom-in-95 duration-200">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${
                      item.status === CrateStatus.GOOD ? 'bg-green-100 text-green-700' :
                      item.status === CrateStatus.DAMAGED ? 'bg-red-100 text-red-700' :
